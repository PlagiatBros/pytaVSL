...
vec2 shake = waveStrength * vec2(waveStrength * 8.0 + 0.5) * vec2(
  random(vec2(rand)) * 2.0 - 1.0,
  random(vec2(rand*2.0)) * 2.0 - 1.0
) / resolution;

float rgbWave = waveStrength * (
      snoise(vec2(relativeY * 0.01, rand * 400.0)) * (2.0 + waveStrength * 32.0)
      * snoise(vec2(relativeY * 0.02, rand * 200.0)) * (1.0 + waveStrength * 4.0)
