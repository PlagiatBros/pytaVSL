/*
    Cellular Tiled Tunnel 2 (https://www.shadertoy.com/view/MdtSRl)
    -----------------------
...
vec4 tunnel( vec2 coords, float speed){

	// Screen coordinates.
	vec2 uv = coords - 0.5;

	// Camera Setup.
	vec3 lookAt = vec3(0., 0.0, unif[12][1]*5.*speed + 0.1);  // "Look At" position.
